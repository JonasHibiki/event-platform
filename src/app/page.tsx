    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="bg-[#111] border border-[#2a2a2a] rounded-t-2xl sm:rounded-xl p-5 w-full sm:max-w-md max-h-[70vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[16px] font-semibold text-[#f5f5f5]">{title}</h3>
...
function LocationDropdown({ value, onChange, cities }: { value: string; onChange: (city: string) => void; cities: readonly string[] }) {
  ...
  const filteredCities = cities.filter(c => c !== 'Other' && c.toLowerCase().includes(search.toLowerCase()))

  return (
    <div ref={containerRef} className="relative">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-[16px] font-medium transition-colors w-full sm:w-auto ${value ? 'bg-[#222] text-[#f5f5f5] border border-[#666]' : 'bg-[#111] border border-[#2a2a2a] text-[#888] hover:border-[#666] hover:text-[#a0a0a0]'}`}
      >
        <MapPinIcon />
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 z-50 bg-[#111] border border-[#2a2a2a] rounded-xl w-full sm:w-[280px] shadow-[0_8px_30px_rgba(0,0,0,0.4)] overflow-hidden">
          {/* Search input */}
          <div className="p-2 border-b border-[#2a2a2a]">
            <div className="relative">
              <div className="absolute left-3 top-1/2 -translate-y-1/2 text-[#666]"><SearchIcon /></div>
              <input
                ref={inputRef}
                type="text"
                value={search}
                ...
                className="w-full pl-9 pr-3 py-2 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg text-[16px] text-[#f5f5f5] placeholder-[#666] focus:outline-none focus:border-[#666]"
              />
            </div>
          </div>
          ...
          <div className="max-h-[300px] overflow-y-auto p-1.5">
            {!search && (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 rounded-lg text-[16px] transition-colors ${!value ? 'bg-[#222] text-[#f5f5f5] font-medium' : 'text-[#a0a0a0] hover:bg-[#1a1a1a]'}`}
              >
                All locations
              </button>
            )}
            {filteredCities.map(c => (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 rounded-lg text-[16px] transition-colors ${value === c ? 'bg-[#222] text-[#f5f5f5] font-medium' : 'text-[#a0a0a0] hover:bg-[#1a1a1a]'}`}
              >
                {c}
              </button>
            ))}
            {filteredCities.length === 0 && (
              <div className="px-3 py-4 text-[14px] text-[#666] text-center">No locations found</div>
            )}
          </div>
        </div>
      )}
...
function DateRangeFilterContent({ startDate, endDate, onApply, onClear }: { startDate: string; endDate: string; onApply: (s: string, e: string) => void; onClear: () => void }) {
  ...
  const formatDisplay = (d: Date | undefined) => {
    if (!d) return '–'
  ...
  return (
    <div>
      ...
      {range?.from && (
        <div className="mt-3 px-1 text-[14px] text-[#a0a0a0]">
          {formatDisplay(range.from)} {range.to ? `– ${formatDisplay(range.to)}` : '– select end date'}
        </div>
      )}

      <div className="flex gap-3 justify-end mt-4">
        <button onClick={() => { setRange(undefined); onClear() }} className="px-4 py-2 text-[16px] font-medium text-[#888] hover:text-[#a0a0a0] transition-colors">Clear</button>
        <button
          onClick={() => {
            if (range?.from) {
              onApply(toStr(range.from), range.to ? toStr(range.to) : toStr(range.from))
            }
          }}
          disabled={!range?.from}
          className="px-5 py-2 bg-[#f5f5f5] text-[#0a0a0a] rounded-lg text-[16px] font-semibold hover:opacity-85 transition-opacity disabled:opacity-40"
        >
          Apply
        </button>
      </div>
    ...
    <Link href={`/events/${event.id}`}>
      <div className="bg-[#111] border border-[#1e1e1e] rounded-xl overflow-hidden hover:border-[#444] transition-colors group">
        <div className="w-full aspect-[4/3] relative bg-[#1a1a1a] overflow-hidden">
          ...
          {event.category && (
            <div className="absolute top-3 left-3" role="img" aria-label={event.category}>
              <span className="flex items-center justify-center w-8 h-8 rounded-full bg-black/55 backdrop-blur-xl text-[#f5f5f5]">
          ...
          {!isUpcoming && (
            <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
              <span className="text-[14px] font-semibold uppercase tracking-wide text-[#a0a0a0]">Ended</span>
            </div>
          )}
        </div>
        <div className="p-4">
          <h3 className="font-semibold text-[16px] text-[#f5f5f5] mb-2.5 line-clamp-2 leading-snug">{event.title}</h3>
          <div className="space-y-1.5 text-[14px] text-[#888]">
          ...
          <div className="flex items-center justify-between mt-3.5 pt-3 border-t border-[#1e1e1e]">
            <span className="text-[14px] text-[#888]">{event._count.rsvps} going</span>
            {event.ticketLink && <span className="text-[14px] text-[#a0a0a0] font-medium">Tickets</span>}
          </div>
        </div>
      </div>
    </Link>
...
export default function HomePage() {
  ...
  // Compute categories with events, sorted by popularity
  ...
  const heading = useMemo(() => {
    // Subject: "{category} events" or just "Events"
    const subject = filters.category ? `${filters.category} events` : 'Events'

    // Location part
    const locationPart = filters.city ? ` in ${filters.city}` : ''

    // Date part
    let datePart = ''
    if (filters.quickFilter === 'today') {
      datePart = ' today'
    } else if (filters.quickFilter === 'weekend') {
      datePart = ' this weekend'
      ...
      if (filters.endDate && filters.endDate !== filters.startDate) {
        datePart = ` ${fmt(filters.startDate)} – ${fmt(filters.endDate)}`
      } else {
        datePart = ` ${fmt(filters.startDate)}`
      }
    }

    // If nothing is selected, show "Upcoming"
    if (!filters.category && !filters.city && !filters.startDate) {
      return 'Upcoming'
    }

    return `${subject}${locationPart}${datePart}`
  }, [filters])

  if (loading) return <div className="min-h-screen flex items-center justify-center"><div className="text-[#888] text-[16px]">Loading events...</div></div>
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-[960px] mx-auto px-5 py-8">
        ...
        {/* ==================== 2. CATEGORY PILLS (only categories with events, sorted by popularity) ==================== */}
          ...
          <div className="mb-5 -mx-5 px-5">
            <div ref={scrollRef} className="flex gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }}>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-full text-[16px] font-medium whitespace-nowrap transition-all flex-shrink-0 ${!filters.category ? 'bg-[#f5f5f5] text-[#0a0a0a]' : 'bg-[#111] border border-[#2a2a2a] text-[#a0a0a0] hover:border-[#666] hover:text-[#f5f5f5]'}`}
              >
                All
              </button>
              {categoriesWithEvents.map(cat => (
                <button
                  key={cat}
                  onClick={() => setFilters(p => ({ ...p, category: p.category === cat ? '' : cat }))}
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-full text-[16px] font-medium whitespace-nowrap transition-all flex-shrink-0 ${filters.category === cat ? 'bg-[#f5f5f5] text-[#0a0a0a]' : 'bg-[#111] border border-[#2a2a2a] text-[#a0a0a0] hover:border-[#666] hover:text-[#f5f5f5]'}`}
                >
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          <button onClick={() => { const d = new Date().toISOString().split('T')[0]; filters.quickFilter === 'today' ? setFilters(p => ({ ...p, startDate: '', endDate: '', quickFilter: '' })) : setFilters(p => ({ ...p, startDate: d, endDate: d, quickFilter: 'today' })) }}
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-[16px] font-medium transition-colors ${filters.quickFilter === 'today' ? 'bg-[#222] text-[#f5f5f5] border border-[#666]' : 'bg-[#111] border border-[#2a2a2a] text-[#888] hover:border-[#666] hover:text-[#a0a0a0]'}`}>
            <SunIcon /> Today
          </button>
          ...
          <button onClick={() => { if (filters.quickFilter === 'weekend') { setFilters(p => ({ ...p, startDate: '', endDate: '', quickFilter: '' })) } else { const { start, end } = getWeekendDates(); setFilters(p => ({ ...p, startDate: start, endDate: end, quickFilter: 'weekend' })) } }}
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-[16px] font-medium transition-colors ${filters.quickFilter === 'weekend' ? 'bg-[#222] text-[#f5f5f5] border border-[#666]' : 'bg-[#111] border border-[#2a2a2a] text-[#888] hover:border-[#666] hover:text-[#a0a0a0]'}`}>
            <WeekendIcon /> This weekend
          </button>

          <button onClick={() => setModals(p => ({ ...p, date: true }))}
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-[16px] font-medium transition-colors ${(filters.startDate || filters.endDate) && !filters.quickFilter ? 'bg-[#222] text-[#f5f5f5] border border-[#666]' : 'bg-[#111] border border-[#2a2a2a] text-[#888] hover:border-[#666] hover:text-[#a0a0a0]'}`}>
            <DateRangeIcon /> Pick dates
          </button>

          {hasFilters && (
            <button onClick={() => setFilters({ category: '', city: '', startDate: '', endDate: '', quickFilter: '' })}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-[16px] font-medium bg-[#1a1a1a] text-[#888] hover:text-[#a0a0a0] transition-colors">
              <XIcon /> Clear all
            </button>
          )}
        </div>

        {hasFilters && <div className="mb-6 text-[14px] text-[#888]">Showing {filteredEvents.length} of {events.length} events</div>}

        {error && <div className="bg-[#111] border border-[#ef4444]/30 rounded-xl p-4 mb-8"><p className="text-[#ef4444] text-[16px]">{error}</p></div>}

        {/* ==================== EVENT GRID ==================== */}
        {upcoming.length > 0 && (
          <div className="mb-12">
            <h2 className="text-[14px] font-semibold uppercase tracking-wider text-[#888] mb-5">{heading}</h2>
        ...
        {past.length > 0 && (
          <div className="mb-12">
            <h2 className="text-[14px] font-semibold uppercase tracking-wider text-[#888] mb-5">Past events</h2>
        ...
        {filteredEvents.length === 0 && events.length > 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-[16px] text-[#888] mb-6">Try different filters</p>
            <button onClick={() => setFilters({ category: '', city: '', startDate: '', endDate: '', quickFilter: '' })} className="bg-[#f5f5f5] text-[#0a0a0a] px-5 py-2.5 rounded-lg text-[16px] font-semibold hover:opacity-85 transition-opacity">Clear filters</button>
          </div>
        )}
        ...
        {events.length === 0 && !loading && (
          <div className="text-center py-20">
            ...
            {canCreate ? (
              <>
                <p className="text-[16px] text-[#888] mb-6">Be the first to create one</p>
                <Link href="/create" className="inline-block bg-[#f5f5f5] text-[#0a0a0a] px-5 py-2.5 rounded-lg text-[16px] font-semibold hover:opacity-85 transition-opacity">Create event</Link>
              </>
            ) : (
              <p className="text-[16px] text-[#888]">Check back soon</p>
            )}
          </div>
        )}
        ...
        <FilterModal isOpen={modals.date} onClose={() => setModals(p => ({ ...p, date: false }))} title="Select date range">
          <DateRangeFilterContent startDate={filters.startDate} endDate={filters.endDate}
